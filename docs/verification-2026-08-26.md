...
The primary backup/share workflow is now a versioned `.phasei-project.zip`. The real exporter was run twice over the same frozen project, company profile/logo, two permission-confirmed manual historical images and one current Toronto 1978 metadata record; the byte streams were identical. The real inspector accepted the artifact without accessing an asset store. The package is 8,626 bytes with SHA-256 `f7d724b427fde645ed243db14406629414436c8b5e6c8b148babe8e39b0ff23a`.
...
Automated verification with the bundled Node runtime and `--test-isolation=none` completed **326/326 tests**, with no failures, cancellations or skips in 8.219 seconds. Task 6 tests cover deterministic export, policy-controlled inclusion, cancellation/no output, mutation-free inspection, raw ZIP structure and path adversaries, size/compression limits, exact manifest/schema/hash/kind/ownership/reference validation, staged commit, stale-state detection, asset collision behavior, compensation at every failure phase, and shared/pre-existing asset preservation. The package dialog tests cover accessible labelling, progress, cancellation/no late download, text-only preview, explicit confirmation, actionable failures, focus trapping/restoration, Escape and mobile CSS. Legacy JSON wiring and labels remain tested.
