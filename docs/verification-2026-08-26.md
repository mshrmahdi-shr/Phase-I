...
## AutoCAD Task 6 UI/package verification — 2026-08-28

The production shared export dialog was exercised through a non-browser JSDOM harness. One set of checked rows, A/C/H-1960-1, drove the production `exportCadPackage`; the UI displayed `3 images selected`, `NAD83 / UTM zone 17N — metres`, and published exactly one completed ZIP. Repeating the complete UI flow produced byte-identical archives. The artifact is 519,920 bytes with SHA-256 `5414489bbdd60dd7363b2ef1752d2770fa17c7e6c069ea181fad270cf1fd84cc`.

The ZIP contains 14 deterministic relative entries: layered `Project.dxf`, the three-page A3 `Combined-Phase-I.pdf`, attachment script, README, source/licence records, CSV/JSON manifests, normalized company logo and three raster/world-file pairs. All 13 non-self manifest byte counts and SHA-256 values matched the extracted files. The DXF has three finite `IMAGE_FRAMES`, `$INSUNITS=6`, required editable layers and `EPSG:26917`. Every world file reconstructs its manifest/DXF CAD frame within `1e-5` metres; independently projected controls are preserved, and each disclosed contextual fit remains within its declared tolerance. No packaged PNG contains a physical-resolution `pHYs` chunk.

Poppler `pdfinfo` confirmed three unencrypted A3 landscape pages, PDF 1.3, with no embedded JavaScript. Poppler rendered all three pages at 110 DPI. Visual review confirmed complete borders, company block/logo, project number `AB-12345`, titles, sources, scale bars, north arrows and page counters without clipping, overlap or broken glyphs. The solid map fields are intentionally injected deterministic verification rasters and do not represent live source imagery.

The successful run composed and disposed A/C/H sequentially, closed all four decoded bitmap handles and returned with zero active surfaces. A final-ZIP abort probe returned `AbortError`, produced no result and also left zero active surfaces. UI regressions separately cover duplicate-click prevention, Cancel/Escape, late completion suppression, actionable failure text, exact disabled-state restoration and object-URL revocation on success and cancellation. Review-specific regressions also keep replacement rows and both PDF/CAD actions locked through a late planner render, enforce the reciprocal PDF-to-CAD lock, defer successful ZIP URL revocation until a later task, and require one immutable exact saved-logo snapshot before CAD becomes ready. The one saved asset-store record and Blob are schema checked, SHA-256 checked and completely decoded; no second asset read can substitute bytes after validation. A substituted Company Profile, missing/extra/wrong metadata keys, invalid hash/timestamp, corrupt or same-ID mutated Blob, and stale readiness generation all fail closed before the package coordinator with re-upload guidance.

The canonical bundled-Node run, `NODE_OPTIONS=--test-isolation=none pnpm test`, passed **417/417 tests** with no failures, cancellations or skips. The production build, changed-source and staged-module syntax checks, and `git diff --check` also passed.

AutoCAD LT 2026 was discovered on the host during the review rerun. Its registered COM automation classes both returned `REGDB_E_CLASSNOTREG`, and a bounded hidden batch attempt to open `Project.dxf`, audit it and save a DWG proof timed out after 60 seconds without producing the proof file. An older unrelated `acadlt` process was left untouched. Therefore no native/compatible open or import is claimed; the DXF result remains structurally parsed and cross-checked only. FreeCAD, LibreCAD, QCAD, ODA converter, DraftSight and BricsCAD were not found. The in-app localhost denial described above also prevented a fresh interactive browser UI run; no workaround, browser screenshot or console-cleanliness claim is made.
