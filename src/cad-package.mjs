...
export async function exportCadPackage({
  ...
  const logoPath=`company/logo.${MIME_EXTENSION[normalizedLogo.mime]}`;
  const imageFrames=ordered.map(value=>({selection:value.selected,corners:value.frame.projectedCorners,pixelWidth:value.normalized.width,pixelHeight:value.normalized.height,imagePath:value.allocated.path})),dxf=buildCadDxf({project:snapshotProject,companyProfile:snapshotProfile,selection:snapshotSelection,imageFrames,projector,companyLogoImage:{path:logoPath,width:normalizedLogo.width,height:normalizedLogo.height}}),dxfBytes=utf8(dxf);throwIfAborted(signal);
  const coreRows=[fileRow('Project.dxf','application/dxf',dxfBytes),fileRow('Combined-Phase-I.pdf','application/pdf',pdfBytes),fileRow(logoPath,normalizedLogo.mime,normalizedLogo.bytes,{pixelWidth:normalizedLogo.width,pixelHeight:normalizedLogo.height})],manifestItems=[];
