...
function checkedFrameCorners(value,label){
  ...
  const corners=value.map((point,index)=>projectedPoint(point,`${label} corner ${index+1}`));
  if(new Set(corners.map(point=>`${point[0]},${point[1]}`)).size!==4)fail(`${label} frame is degenerate or repeats a corner.`);
  ...
  const scale=Math.max(Math.hypot(...column),Math.hypot(...row));
  const determinant=column[0]*row[1]-column[1]*row[0];
  if(!(scale>0)||!Number.isFinite(determinant)||Math.abs(determinant)<=scale*scale*1e-12)fail(`${label} frame is degenerate or ill-conditioned.`);
  const expected=[upperRight[0]+lowerLeft[0]-upperLeft[0],upperRight[1]+lowerLeft[1]-upperLeft[1]],tolerance=Math.max(1e-5,scale*1e-9);
  if(Math.hypot(lowerRight[0]-expected[0],lowerRight[1]-expected[1])>tolerance)fail(`${label} corners do not form one affine raster frame.`);
  return corners;
}
...
function projectedGeometry(project,projector,frames){
  ...
  const site=project.siteBoundary.length?projectRing(project.siteBoundary,projector):[];
  const building=project.buildingBoundary.length?projectRing(project.buildingBoundary,projector):[];
  const marker=projectedPoint(projector.forward([project.source.location.lng,project.source.location.lat]),'Projected SITE marker');
...
function titleLayout(bounds,selectionCount,company){
  ...
  const logoWidth=Math.min(55,width*.28),logoHeight=Math.min(30,logoWidth*company.logoHeight/company.logoWidth);
  return {x,y,width,height,outer:rectangle(x,y,width,height),logo:rectangle(x+6,y+height-6-logoHeight,logoWidth,logoHeight)};
}

function trimClosure(points){
...
function numberText(value){
  finite(value,'DXF number');const clean=value===0?0:Number(value.toPrecision(12));
  if(!Number.isFinite(clean))fail('DXF number exceeds the supported range.');return String(clean);
}
...
function pointEntity(writer,handles,owner,layerName,[x,y]){
  entityStart(writer,handles,owner,'POINT',layerName,'AcDbPoint');writer.add(10,x).add(20,y).add(30,0);
}

function polylineEntity(writer,handles,owner,layerName,points,{closed=true}={}){
  const vertices=closed?trimClosure(points):points.map(point=>[...point]);
  if(vertices.length<(closed?3:2)||vertices.length>5000)fail(`${layerName} polyline has an invalid vertex count.`);
  ...
  for(const point of vertices){const [x,y]=projectedPoint(point,`${layerName} polyline vertex`);writer.add(10,x).add(20,y);}
}
...
function entitiesModel(project,company,selections,frames,geometry,layout){
  ...
  for(const frame of frames){
    result.push({kind:'polyline',layer:'IMAGE_FRAMES',points:frame.corners,closed:true});
  ...
  result.push({kind:'polyline',layer:'TITLE_BLOCK',points:layout.outer,closed:true});
  ...
  result.push({kind:'polyline',layer:'COMPANY_LOGO_FRAME',points:layout.logo,closed:true});
  const textX=layout.x+Math.min(68,layout.width*.34),textWidth=layout.width-(textX-layout.x)-6;
...
function buildEntities(handles,owner,model){
  ...
  for(const entity of model){
    ...
    else if(entity.kind==='polyline')polylineEntity(writer,handles,owner,entity.layer,entity.points,{closed:entity.closed});
