...
export function createExportDialog({document,getState,save,setBusy,exportPdf,planPdf=null,download=downloadPdf,exportCadPackage=null,downloadCad}){
  const byId=id=>document.getElementById(id),dialog=byId('exportDialog'),sheetCount=count=>`${count} ${count===1?'sheet':'sheets'}`;
  let controller=null,planning=null,planningController=null,planStates=new Map(),planGeneration=0,returnFocus=null,background=[],cadController=null;
  ...
  function cadSnapshot(){
    ...
    return {...state,selection:exact,ready:exact.length===selected.length&&exact.length>0&&blockers.length===0,blockers};
  }
  ...
  function render(){
    ...
    const physicalCount=selected.reduce((sum,row)=>sum+1+(planStates.get(row.key)?.continuations||0),0);byId('downloadPdf').textContent=`Download PDF (${sheetCount(physicalCount)})`;byId('downloadPdf').disabled=!selected.length||selected.length!==retained.length;cadController?.refresh();if(focused?.startsWith('exportFigure')||focused?.startsWith('exportHistorical'))byId(focused)?.focus();
  }
  function refresh(){
  ...
  if(exportCadPackage)cadController=createCadExportController({document,getSnapshot:cadSnapshot,setBusy,exportPackage:exportCadPackage,...(downloadCad?{download:downloadCad}:{})});
