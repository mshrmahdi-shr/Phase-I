...
export function createExportDialog({document,getState,save,setBusy,exportPdf,planPdf=null,download=downloadPdf,exportCadPackage=null,downloadCad,prepareCadBranding=null}){
  ...
  function checkedBranding(value,expected){
    if(!value||typeof value!=='object'||!value.companyProfile||!value.companyLogo)throw new Error('The saved company logo is missing. Upload the PNG or JPEG logo again.');const profile=structuredClone(value.companyProfile),logo=value.companyLogo,metadata=structuredClone(logo.metadata);
    if(!(logo.blob instanceof Blob))throw new Error('The saved company logo is missing. Upload the PNG or JPEG logo again.');if(metadata?.id!==profile.logoAssetId||metadata?.id!==expected.logoAssetId||metadata?.kind!=='company-logo'||metadata?.mime!==profile.logoMime||metadata?.width!==profile.logoWidth||metadata?.height!==profile.logoHeight||metadata?.size!==logo.blob.size||logo.blob.type!==profile.logoMime)throw new Error('The saved company logo metadata does not match the profile. Upload the logo again.');return {companyProfile:profile,companyLogo:{metadata,blob:logo.blob}};
  }
  function beginBranding(state,generation){
    ...
    brandingPlanning=Promise.resolve().then(()=>{if(typeof prepareCadBranding!=='function')throw new Error('Company logo readiness check is unavailable. Reopen Company Profile and save the logo again.');return prepareCadBranding(structuredClone(state.companyProfile),{signal:pending.signal});}).then(value=>{if(generation!==planGeneration||pending.signal.aborted)return;brandingState={status:'ready',value:checkedBranding(value,state.companyProfile)};render();}).catch(error=>{if(generation!==planGeneration||pending.signal.aborted)return;brandingState={status:'error',error};render();}).finally(()=>{if(generation===planGeneration&&brandingController===pending){brandingController=null;brandingPlanning=null;}});return brandingPlanning;
  }
