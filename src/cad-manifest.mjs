...
function readme(project,crs){
  return [
    ...
    'GET STARTED','1. Extract All files from the ZIP before opening anything. Keep the folder structure together.',
    '2. Open Project.dxf in AutoCAD or a compatible CAD program.','3. In AutoCAD, run the SCRIPT command.',
    '4. Choose Attach-Images.scr from the extracted package. The script attaches each image at its generated metre coordinates.',
    '5. If an image is missing, use External References to relink the images folder (and the company folder for the logo).',
    '6. Edit, move, scale, rotate, clip, detach, or replace the common entities and raster references as needed.','',
