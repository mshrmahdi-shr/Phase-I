...
const IMAGE_SERVICE=new URL('Ontario_Imagery_Web_Map_Service/MapServer',ROOT).href;
...
async function search({location,signal,fetchImpl=globalThis.fetch}){
  ...
  const [source,imagery]=await Promise.all([
    fetchArcGisJson(SOURCE_SERVICE,options),fetchArcGisJson(IMAGE_SERVICE,options)
  ]);
  if(!Array.isArray(source.layers))throw new TypeError('Ontario imagery source layers are missing');
  const operation=arcGisServiceExport(imagery);
  ...
  const values=await Promise.all(entries.map(async layer=>{
    ...
    const policy=operation?'exportable':'link-only';
    return {
      id:collection.id,providerId:'ontario',title:collection.title,year:collection.year,
      resolutionMeters:null,coverage,
      preview:{kind:'arcgis-map-service',url:IMAGE_SERVICE,layer:0,tileTemplate:`${IMAGE_SERVICE}/tile/{z}/{y}/{x}`},
      export:policy==='exportable'?{kind:'arcgis-export',url:`${IMAGE_SERVICE}/export`,layer:0,maxWidth:operation.maxWidth,maxHeight:operation.maxHeight}:null,
      policy,sourceUrl,licenseUrl:LICENSE_URL,attribution:'Government of Ontario'
    };
  }));
