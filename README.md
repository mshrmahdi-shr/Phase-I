...
2. B requires a completed site boundary. D/E require loaded geometry, coverage of the fitted sheet and a detected SITE unit. Use the official load buttons or import self-contained polygon KML/KMZ. A custom import opens a source-details form: enter the actual organization credits, licence/terms and permission evidence, then explicitly confirm your rights. Leave the optional acquisition/publication year blank when unknown; the app never guesses it. **Edit custom source details** corrects saved provenance without another upload. Selecting D/E loads missing official data; custom datasets are never silently replaced with official data. H requires approved, placed historical imagery and an unchanged company-profile snapshot.
...
4. Press **Download PDF (N sheets)**. Editing is locked during composition. Cancel or Escape stops the batch; no partial PDF downloads. On success, the browser downloads one file with original figure letters and separate page counters.
   The adjacent **Download AutoCAD ZIP (N images)** action uses those exact checked rows; its count is raster images, not PDF sheets. The dialog previews the SITE-selected NAD83 / UTM zone and blocks incomplete company/logo, missing asset, stale official source, link-only/unknown policy, unsafe projection input, or missing custom-geology legal provenance. A valid PDF selection can remain downloadable while CAD explains the additional missing source details.
